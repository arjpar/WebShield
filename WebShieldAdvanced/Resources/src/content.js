...
(async () => {
  ...
  const applyScriptlets = async (scriptletsData) => {
    ...
    scriptletPayloads.forEach(({ code, source, args }) => {
      const executionCode = `
        (function() {
          console.info("[WebShield Advanced] Starting scriptlet: ${source.name}");
          ${code}
          try {
            var fn = window.adguardScriptlets["${source.name}"];
            if (typeof fn !== "function") {
              throw new Error("[WebShield Advanced] Scriptlet function not found for ${source.name}");
            }
            // Wrap the call in a try-catch to log any synchronous errors.
            try {
              fn(${JSON.stringify(source)}, ...${JSON.stringify(args)});
              console.info("[WebShield Advanced] Successfully executed scriptlet: ${source.name}");
            } catch(innerError) {
              console.error("[WebShield Advanced] Error during scriptlet call (" + "${source.name}" + "):", innerError);
            }
          } catch (e) {
            console.error("[WebShield Advanced] Scriptlet error for ${source.name}:", e);
          }
        })();
      `;
